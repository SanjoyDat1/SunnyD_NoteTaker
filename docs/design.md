...
Three levels, all warm-tinted (never pure black shadows): **e1** for resting cards in the annotation rail, **e2** for hover states and anchored popovers, **e3** for modals and floating players. A surface's shadow tells the user how "above the page" it is — resting content whispers, focused surfaces speak, modals command.
